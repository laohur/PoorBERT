...
class AlbertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.task_type_embeddings = nn.Embedding(config.task_type_size, config.embedding_size)
        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        self.LayerNorm = NORM2FN[config.norm](config.embedding_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids, task_type_ids=None, position_ids=None):
        ...
        embeddings = words_embeddings + position_embeddings + task_type_embeddings
...
class AlbertLayer(nn.Module):
    ...
    def forward1(self, hidden_states, attention_mask=None, head_mask=None):
        global t0
        t0=time_cost("before atention")
        for i in range(100):
            attention_outputs = self.attention(hidden_states, attention_mask, head_mask)
        # attention_output = self.LayerNorm(attention_outputs[0] + hidden_states)
        t0=time_cost("atention")
        for i in range(100):
            attention_output = self.LayerNorm(attention_outputs[0] )+hidden_states
        t0=time_cost("LayerNorm")
        for i in range(100):
            ffn_output = self.ffn(attention_output)
        t0=time_cost(" ffn")
        # ffn_output = self.LayerNorm_1(ffn_output+attention_output)
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, task_type_ids=None, position_ids=None, head_mask=None):
        ...
        if task_type_ids is None:
            # task_type_ids = torch.zeros_like(input_ids)
            task_type_ids = torch.zeros((batch,seq),dtype=input_ids.dtype,device=input_ids.device)
        ...
        embedding_output = self.embeddings(input_ids, position_ids=position_ids, task_type_ids=task_type_ids)
...
class AlbertForPreTraining(AlbertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, task_type_ids=None, position_ids=None, head_mask=None,
        ...
        # pooled2 = self.norm(seq2[:, 0, :])  # layer-2  pooled output
        pooled2 = self.activation(seq2[:, 0, :])  # layer-2  pooled output
        logits={}
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, task_type_ids=None,
